...
class Dashboard(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.canvas.tag_bind(alarm_image_obj, "<ButtonRelease-1>",
                             lambda event: (flash_hidden(alarm_image_obj), controller.show_canvas(Alarms)))
...
class Alarms(tk.Frame):
    """
    Configures, and displays the Alarms page
    """
    ...
    def __init__(self, parent, controller):
        ...
        self.alerts_background_img = PhotoImage(file = f"Collection of all UI Graphics/alerts_page_background.png")
        alerts_background = self.canvas.create_image( 720.0, 512.0, image=self.alerts_background_img)
        
        self.create_alerts = PhotoImage(file = f"Collection of all UI Graphics/create_alert.png")
        create_alerts_button = self.canvas.create_image(1130, 160, anchor='nw', image=self.create_alerts)
        self.canvas.tag_bind(create_alerts_button, "<ButtonRelease-1>",
                             lambda event: [flash_hidden(create_alerts_button), notifications_clicker(self)])

        # canvas.create_text( 350.5, 307.0, text = "Bitcoin - BTC", fill = "#abb0c8", font = ("Rosarivo-Regular", int(13.0)))
        # canvas.create_text( 554.0, 305.0, text = "<menu option> ", fill = "#abb0c8", font = ("Rosarivo-Regular", int(13.0)))
        # canvas.create_text( 720.0, 308.0, text = "$1.89", fill = "#abb0c8", font = ("Rosarivo-Regular", int(13.0)))
        # canvas.create_text( 867.0, 306.0, text = "One Time", fill = "#abb0c8", font = ("Rosarivo-Regular", int(13.0)))
        # canvas.create_text( 1074.5, 307.0, text = "Profit Margin #1 for +22.33%", fill = "#abb0c8", font = ("Rosarivo-Regular", int(13.0)))
        ...
        # Retrieves the images, and configures the alerts button
        alarm_image_path = "Collection of all UI Graphics/dashboard_alarms.png"
        self.alarm_image = tk.PhotoImage(file=alarm_image_path)
        alarm_image_obj = self.canvas.create_image(0, 560, anchor='nw', image=self.alarm_image)
        self.canvas.tag_bind(alarm_image_obj, "<ButtonRelease-1>",
        ...
        news_image_obj = self.canvas.create_image(0, 670, anchor='nw', image=self.news_image)
        self.canvas.tag_bind(news_image_obj, "<ButtonRelease-1>",
                             lambda event: (flash_hidden(news_image_obj), controller.show_canvas(Alarms)))
